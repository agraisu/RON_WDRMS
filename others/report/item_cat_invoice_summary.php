    ...
    <body>
        <form method="post" action="">
            <table class="tbl_boder displayHide center" border="" style="width: 100%; font-weight: bold;background-color: white">
                <thead>
                    <tr>
                        <th class="text-center mr-5">Select Category : </th>
                        <th><select class="form-control col-md-5" style="background-color: #D6E9C6" name="category">
                ...
                <thead>
                    <tr>
                        ...
                        <td style="text-align: left;">
                            ...
                            <span>Selected Category </span><span>: <?php
                                if ($_POST['category'] != 0) {
                                    $cat_name = "SELECT cat_name FROM `item_category_details` WHERE item_category_details.cat_id = '{$_POST['category']}'";
                                    $cat_res = $app->basic_Select_Query($cat_name);
                                    echo @$cat_res[0]['cat_name'];
                                } else {
                                    echo 'All Category';
                                }
                                ?></span><br>
            ...
            <table class="center" border="1" style="width: 100%; font-size: 13pt; font-weight: bold;background-color: white" class="tbl_boder">
                <tr>
                    ...
                    <?php if ($_POST['category'] == 0) { ?>
                        <th style="width: 5cm;">Categorys</th>
                    <?php } ?>
                    <th style="width: 5cm;">INV Qty</th>
...
(invoice_item_details.inv_itm_qty*invoice_item_details.inv_unit_price) AS itm_tot,
...
INNER JOIN invoice_details ON invoice_details.inv_no = invoice_item_details.invoice_no
...
DATE_FORMAT(invoice_details.inv_added_date_time,'%Y-%m-%d') BETWEEN '{$_POST['from']}' AND '{$_POST['to']}' AND
 item_category_details.cat_id = '{$_POST['category']}' AND
...
invoice_details.inv_status = 2";
                    ...
                    foreach ($result AS $aa) {
                        ...
                        . '<td>' . $aa['inv_no'] . '</td>';
                        if ($_POST['category'] == 0) {
                            echo '<td>' . $aa['cat_name'] . '</td>';
                        }
                        echo '<td>' . $aa['inv_itm_qty'] . '</td>'
                        . '<td>' . $aa['inv_added_date_time'] . '</td>'
                        . '<td>' . $aa['cus_name'] . '</td>'
                        . '<td>' . $aa['full_name'] . '</td>'
                        . '<td style="width: 5cm; text-align: right; padding-left: 15px; font-weight: bold; font-size: 18px;">' . $aa['itm_tot'] . '&nbsp</td>'
                        . '</tr>';

                        $total_inv += $aa['itm_tot'];
                    ...
                    echo '<tr>';
                    echo '<td colspan="6" style="font-weight: bold; font-size: 20px; text-align: right;">Totals : &nbsp;</td>';
